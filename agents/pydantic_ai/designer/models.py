...
class DesignRecommendation(BaseModel):
    ...
    ] = Field(description="Design recommendation category")
    ...
    user_impact: Literal["high", "medium", "low"] = Field(
        description="Expected user experience impact"
    )
    design_principles: List[str] = Field(
        default_factory=list, description="Design principles this addresses"
    )
...
class AccessibilityFinding(BaseModel):
    ...
    severity: Literal["critical", "high", "medium", "low"] = Field(
        description="Accessibility issue severity"
    )
    affected_components: List[str] = Field(
        default_factory=list, description="UI components affected"
    )
    remediation_steps: List[str] = Field(
        default_factory=list, description="Steps to fix accessibility issue"
    )
    testing_instructions: str = Field(
        description="How to test the accessibility improvement"
    )
...
class DesignSystemComponent(BaseModel):
    ...
    component_type: Literal["atom", "molecule", "organism", "template", "page"] = Field(
        description="Atomic design hierarchy level"
    )
    description: str = Field(description="Component purpose and usage")
    design_tokens: Dict[str, str] = Field(
        default_factory=dict,
        description="Design tokens used (colors, typography, spacing)",
    )
    states: List[str] = Field(
        default_factory=list,
        description="Component states (default, hover, active, disabled)",
    )
    variants: List[str] = Field(
        default_factory=list, description="Component variants or sizes"
    )
    accessibility_features: List[str] = Field(
        default_factory=list, description="Built-in accessibility features"
    )
    implementation_notes: str = Field(
        default="", description="Technical implementation guidance"
    )
