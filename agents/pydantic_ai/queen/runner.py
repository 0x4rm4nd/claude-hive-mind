...
=========================
Execution runner for the Queen Orchestrator - provides multi-worker coordination and continuous monitoring.
"""
...
from queen.models import QueenOutput, QueenOrchestrationPlan, WorkerAssignment
...
class QueenWorker(BaseWorker[QueenOutput]):
    ...
    def run(self, session_id: str, task_description: str, model: str) -> QueenOutput:
        ...
        self.worker_config = QueenAgentConfig.create_worker_config(session_id, task_description)
        return self.run_analysis(session_id, task_description, model)
        
    def execute_ai_analysis(self, session_id: str, task_description: str, model: str) -> Any:
        """Execute queen-specific orchestration analysis"""
        # Check if monitoring mode is requested
        monitoring_mode = "--monitor" in task_description or "monitor" in task_description.lower()
        
        # Run orchestration using existing function
        orchestration_plan = run_orchestration(session_id, task_description, model)
        
        # Create QueenOutput from orchestration plan
        ...
        queen_output = QueenOutput(
            ...
            status="completed",
            summary={
                "key_findings": [f"Orchestration plan generated with {len(orchestration_plan.worker_assignments)} workers"],
                "critical_issues": [],
                "recommendations": [assignment.rationale for assignment in orchestration_plan.worker_assignments]
            },
            orchestration_plan=orchestration_plan,
            workers_spawned=[assignment.worker_type for assignment in orchestration_plan.worker_assignments],
    ...
    def get_success_message(self, output: QueenOutput) -> str:
        return (f"Queen orchestration completed. Workers spawned: {len(output.workers_spawned)}, "
                f"Coordination status: {output.coordination_status}")
        
    def create_worker_specific_files(self, session_id: str, output: QueenOutput, session_path: Path) -> None:
        ...
        orchestration_file.write_text(output.orchestration_plan.model_dump_json(indent=2))
...
def main():
    """CLI entry point for queen worker execution.
...
def legacy_main():
    ...
    # Add monitoring flag to task if needed
    task_description = args.task
    if args.monitor:
        task_description += " --monitor"

    # Use new QueenWorker class
    worker = QueenWorker()
    try:
        output = worker.run(args.session, task_description, args.model)
        print(f"Queen orchestration completed: {worker.get_success_message(output)}")
        
        # If monitoring enabled, start continuous monitoring
        if args.monitor and output.orchestration_plan.worker_assignments:
            print(
            ...
            asyncio.run(
                monitor_worker_progress(
                    args.session,
                    output.orchestration_plan.worker_assignments,
