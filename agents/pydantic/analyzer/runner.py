...
def log_debug(session_id: str, message: str, details: Any):
    ...
    try:
        cfg = ProtocolConfig({"session_id": session_id, "agent_name": "analyzer-worker"})
        logger = LoggingProtocol(cfg)
...
Provide specific, actionable findings with clear priorities and effort estimates.""",
        ...
        log_debug(
            session_id, "Analyzer analysis failed", {"error": str(e), "task": task_description}
        )
...
def create_analyzer_files(session_id: str, output: AnalyzerOutput):
    ...
    try:
        ...
        if output.notes_markdown:
            ...
            log_debug(session_id, "Created analyzer notes file", {"path": str(notes_file)})
        ...
        log_debug(session_id, "Created analyzer output JSON", {"path": str(output_file)})
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyzer Worker - Security and Performance Analysis")
    ...
    try:
        ...
        print(f"Analysis completed successfully. Security score: {output.security_score}, Performance score: {output.performance_score}, Quality score: {output.quality_score}")
        return 0
    except Exception as e:
